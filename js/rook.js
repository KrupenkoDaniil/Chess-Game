...
export const rookCheck = (color, pos_x, pos_y) => {
    ...
    const horizontalCells = document.querySelectorAll(`[data-y='${pos_y}'].game-board__cell`);
    const verticalCells = document.querySelectorAll(`[data-x='${pos_x}'].game-board__cell`);

    // Right direction
    let searchingRight = true;
    let c = 0;
    while (searchingRight) {
        if ((horizontalCells[c].dataset.x > pos_x)) {
            horizontalCells[c].classList.add('game-board__cell_active');
        } if ((horizontalCells[c].dataset.x > pos_x)
            && (board[pos_y - 1][c] !== null)
            && (board[pos_y - 1][c][0] == color)) {
            horizontalCells[c].classList.remove('game-board__cell_active');
            searchingRight = false;
        } if ((horizontalCells[c].dataset.x > pos_x)
            && (board[pos_y - 1][c] !== null)
            && (board[pos_y - 1][c][0] !== color)) {
            let activeFigure = document.querySelector(`[data-x='${c + 1}'][data-y='${pos_y}'].figure`);
    ...
    while (searchingDown) {
        if ((verticalCells[c].dataset.y > pos_y)) {
            ...
            && (board[c][pos_x - 1] !== null)
            && (board[c][pos_x - 1][0] == color)) {
