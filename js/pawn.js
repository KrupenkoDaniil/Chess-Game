...
export const pawnCheck = (color, pos_x, pos_y) => {
    ...
    if (color == 'w') {
        ...
        for (let c = 0; c < checkCellsAhead.length; c++) {
            if ((checkCellsAhead[c].dataset.y == pos_y - 1) // if it is the right  coords
                && (checkCellsAhead[c].classList.contains('game-board__cell')) // if it is the cell
                && (board[pos_y - 2][pos_x - 1] == null)) { // if it is empty
                checkCellsAhead[c].classList.add('game-board__cell_active');
            } if ((checkCellsAhead[c].dataset.y == pos_y - 1) // Checking if enemy Ahead
                && (checkCellsAhead[c].classList.contains('game-board__cell'))
                && (board[pos_y - 2][pos_x - 1] !== null)) {
                enemyAhead = true;
            }
        }
        ...
        if (pos_y == 7 && !enemyAhead) {
            for (let c = 0; c < checkCellsAhead.length; c++) {
                if ((checkCellsAhead[c].dataset.y == pos_y - 2)
                    && (checkCellsAhead[c].classList.contains('game-board__cell'))
                    && (board[pos_y - 3][pos_x - 1] == null)) {
                    checkCellsAhead[c].classList.add('game-board__cell_active');
                }
            }
        }
        ...
        if (checkCellsOnLeft.length !== 0) {
            for (let c = 0; c < checkCellsOnLeft.length; c++) {
                if (
                    (checkCellsOnLeft[c].dataset.y == pos_y - 1)
                    && (checkCellsOnLeft[c].classList.contains('game-board__cell'))
                    && (board[pos_y - 2][pos_x - 2] !== null)
                    && (board[pos_y - 2][pos_x - 2][0] !== 'w') // Checking if it is ally 
                ) {
        ...
        if (checkCellsOnRight.length !== 0) {
            for (let c = 0; c < checkCellsOnRight.length; c++) {
                if (
                    (checkCellsOnRight[c].dataset.y == pos_y - 1)
                    && (checkCellsOnRight[c].classList.contains('game-board__cell'))
                    && (board[pos_y - 2][pos_x] !== null)
                    && (board[pos_y - 2][pos_x][0] !== 'w') // Checking if it is ally 
                ) {
        ...
        for (let c = 0; c < checkCellsAhead.length; c++) {
            if ((checkCellsAhead[c].dataset.y == pos_y + 1) // if it is the right  coords
                && (checkCellsAhead[c].classList.contains('game-board__cell')) // if it is the cell
                && (board[pos_y][pos_x - 1] == null)) { // if it is empty
                checkCellsAhead[c].classList.add('game-board__cell_active');
            } if ((checkCellsAhead[c].dataset.y == pos_y + 1) // Checking if enemy Ahead
                && (checkCellsAhead[c].classList.contains('game-board__cell'))
                && (board[pos_y][pos_x - 1] !== null)) {
                enemyAhead = true;
            }
        }
        // check if it is the first move for pawn
        ...
        if (checkCellsOnLeft.length !== 0) {
            for (let c = 0; c < checkCellsOnLeft.length; c++) {
                if ((checkCellsOnLeft[c].dataset.y == pos_y + 1)
                    && (checkCellsOnLeft[c].classList.contains('game-board__cell'))
                    && (board[pos_y][pos_x - 2] !== null)
                    && (board[pos_y][pos_x - 2][0] !== 'b')) { // Checking if it is foe 
                    checkCellsOnLeft[c].classList.add('game-board__cell_active');
                } if ((checkCellsOnLeft[c].dataset.y == pos_y + 1)
                    && (checkCellsOnLeft[c].classList.contains('figure'))
                    && (board[pos_y][pos_x - 2] !== null)
        ...
        if (checkCellsOnRight.length !== 0) {
            for (let c = 0; c < checkCellsOnRight.length; c++) {
                if ((checkCellsOnRight[c].dataset.y == pos_y + 1)
                    && (checkCellsOnRight[c].classList.contains('game-board__cell'))
                    && (board[pos_y][pos_x] !== null)
                    && (board[pos_y][pos_x][0] !== 'b')) { // Checking if it is foe 
                    checkCellsOnRight[c].classList.add('game-board__cell_active');
                } if ((checkCellsOnRight[c].dataset.y == pos_y + 1)
                    && (checkCellsOnRight[c].classList.contains('figure'))
                    && (board[pos_y][pos_x] !== null)
